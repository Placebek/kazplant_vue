...
const fetchPlant = async () => {
  try {
    ...
    if (result.success) {
      ...
      plantsStore.error = result.error || 'Не удалось загрузить данные растения'
    }
  } catch (error) {
    plantsStore.error = 'Ошибка при загрузке данных: ' + error.message
  }
}

// Запрос на сегментацию изображения
const toSegmentation = async (imagePath) => {
  if (!plant.value || !imagePath) {
    segmentationError.value = 'Данные растения или изображение недоступны'
    return
  }
  ...
  try {
    const fullImageUrl = `https://pystorm.space/${imagePath}`
    if (window.ok?.segmentDiseaseClick) {
      // Ожидаем завершения сегментации через Promise
      const result = await new Promise((resolve, reject) => {
        window.ok.segmentDiseaseClick(fullImageUrl)
        // Устанавливаем таймаут на случай, если сегментация зависнет
        setTimeout(() => reject(new Error('Таймаут сегментации')), 10000)
      })
      segmentedData.value = result
    } else {
      throw new Error('Функция сегментации недоступна')
    }
  } catch (error) {
    segmentationError.value = 'Ошибка сегментации: ' + error.message
  } finally {
    modal.value = false
  }
}
